...
fn read_and_compute_by_line<T: Read>(reader: T) -> io::Result<()> {
    ...
    // lazy binary search solution lol
    let mut left = 1;
    let mut right = 4194304; // this value was found first by running the same loop, but doubling the fuel at every step until it passed the wanted fuel :)
    while left <= right {
        let fuel = (left + right) / 2;
        let mut copy = dep_graph.clone();
        let cost = copy.cost(String::from("FUEL"), fuel);
        ...
        if cost > 1000000000000 {
            ...
            // we have a match! We might need to adjust for leftovers on nodes...
            println!("Did we find it? {}", fuel);
            break;
        }

        // The last one may be below the trillion, meaning that we need more than a trillion to produce more.
        println!("{} - {}", fuel, cost);
    }
